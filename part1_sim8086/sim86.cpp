...
int main(int ArgCount, char **Args)
{
    ...
    if(ArgCount > 1)
    {
        if (ArgCount == 2){
            ...
            u32 ArgI = 1;
            bool DumpFlag = false;
            if (0 == strcmp(Args[ArgI],"--dump")) {
                DumpFlag = true;
                ArgI ++;
            }

            if (0 == strcmp(Args[ArgI],"--exec")) {
                char *FileName = Args[ArgI + 1];
                u32 BytesRead = LoadMemoryFromFile(FileName, Memory, 0);

                printf("\n; %s simulation:\n", FileName);
                ExecAsm8086(Memory, {}, BytesRead);
            }

            if (DumpFlag) {
                FILE *fp = fopen("dump.data", "wb");
                fwrite(Memory->Bytes, 1, ArrayCount(Memory->Bytes), fp);
                fclose(fp);
            }
        }
    }
